...
function ForensicMap({ threat }: { threat: Threat }) {
  ...
  const isInsider = threat.sourceIP?.startsWith('192.168.20.') || threat.sourceIP?.startsWith('192.168.10.');
  // Outside: internet top, router below, pi center, stage1/iptables right, stage2/dell bottom-right
  // Inside:  attacker bottom-left (VLAN20 device), switch above attacker, pi center, attacker path goes up through switch to pi
  const ATTACKER = isInsider ? { x: 120, y: 260 } : { x: 60,  y: 155 };
  const ROUTER   = isInsider ? { x: 120, y: 155 } : { x: 210, y: 155 };
  const PI       = { x: 380, y: 155 };
  const STAGE1   = { x: 500, y: 60  };
  const IPTABLES = { x: 660, y: 60  };
  const STAGE2   = { x: 500, y: 260 };
  const DELL     = { x: 660, y: 260 };

  // Attack path: insider comes from bottom-left up through switch to pi
  const atkPath = isInsider
    ? 'M' + ATTACKER.x + ',' + ATTACKER.y + ' L' + ROUTER.x + ',' + (ROUTER.y + 16) + ' L' + (PI.x - 26) + ',' + PI.y
    : 'M' + ATTACKER.x + ',' + ATTACKER.y + ' Q' + ((ATTACKER.x+PI.x)/2) + ',' + (ATTACKER.y - 60) + ' ' + PI.x + ',' + PI.y;

  const stage2Active = !isBlocked;
  const nodes = [
    { key: 'attacker', ...ATTACKER, icon: '💀', label: 'ATTACKER', sub: threat.sourceIP, sub2: isInsider ? 'VLAN 20 insider' : threat.type, color: '#ff3300', border: '#ff4d4d', bg: '#1a0000', glow: true, detail: isInsider ? 'Insider threat\n' + threat.sourceIP + ' on untrusted VLAN 20' : 'External attacker\n' + threat.sourceIP + ' from internet' },
    { key: 'router',   ...ROUTER,   icon: isInsider ? '🔀' : '📡', label: isInsider ? 'SWITCH' : 'ROUTER', sub: isInsider ? 'Cisco 3560' : '192.168.0.1', sub2: isInsider ? 'VLAN 20 → Pi 5' : 'forwarded', color: '#4a7fa5', border: '#4a7fa5', bg: '#041520', glow: false, detail: isInsider ? 'Cisco Catalyst 3560\nAttacker on untrusted VLAN 20' : 'Traffic forwarded\nNo deep packet inspection' },
...
export default function IncidentInvestigation({ threat, logs, onBack }: IncidentInvestigationProps) {
  const [activeTab, setActiveTab] = useState<'forensics' | 'signals' | 'timeline' | 'cve' | 'chat'>('forensics');
  const sc = SEV_COLOR[threat.severity] ?? '#8899aa';
  const stc = STATUS_COLOR[threat.status] ?? '#8899aa';
  const stb = STATUS_BG[threat.status] ?? '#1a2540';
  ...
  return (
    <>
      ...
      <div style={{ padding: '20px 24px' }}>
        ...
        {activeTab === 'cve' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={{ fontFamily: 'monospace', fontSize: 13, color: '#4a7fa5', letterSpacing: 2, marginBottom: 4 }}>CVE INTELLIGENCE — SCANNED PORTS</div>
            {(!threat.cve_matches || threat.cve_matches.length === 0) ? (
              <div style={{ fontFamily: 'monospace', fontSize: 13, color: '#2a4a6a', padding: 20, textAlign: 'center' }}>
                No CVE matches found for scanned ports
              </div>
            ) : (
              threat.cve_matches.map((cve, i) => (
                <div key={i} style={{
                  background: '#06111f', border: '1px solid #1e3050', borderRadius: 6,
                  padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 6,
                }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontFamily: 'monospace', fontSize: 13, color: '#ff4d4d', fontWeight: 700 }}>{cve.cve_id}</span>
                    <span style={{ fontFamily: 'monospace', fontSize: 12, color: cve.severity === 'critical' ? '#ff4d4d' : cve.severity === 'high' ? '#ff8c42' : '#ffd166', fontWeight: 700 }}>
                      CVSS {cve.cvss_score} — {cve.severity?.toUpperCase()}
                    </span>
                  </div>
                  <div style={{ fontFamily: 'monospace', fontSize: 12, color: '#c8d6e5' }}>{cve.description}</div>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span style={{ fontFamily: 'monospace', fontSize: 11, color: '#4a7fa5' }}>Port {cve.port} — {cve.service}</span>
                    <a href={cve.url} target="_blank" rel="noreferrer" style={{ fontFamily: 'monospace', fontSize: 11, color: '#7f77dd' }}>NVD ↗</a>
                  </div>
                </div>
              ))
            )}
